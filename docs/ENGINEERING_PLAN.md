...
**Status:** In build — M0–M2 + M4 enrichment complete (foundations → extraction → gold → indication/therapeutic-area/target normalization, with disclosed-first provenance). Eval gate (M3) sequenced before scale-out; entity resolution deferred to scale. See §0.
