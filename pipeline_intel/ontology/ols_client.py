...
# Disease anchors live in these ontologies; upper-ontology (BFO, etc.) ancestors are noise.
DISEASE_PREFIXES = ("EFO:", "MONDO:", "Orphanet:", "HP:", "DOID:")
...
def term_by_curie(curie: str) -> TermCandidate | None:
    ...
    with _client() as c:
        r = c.get(f"{OLS_BASE}/ontologies/efo/terms", params={"obo_id": curie, "size": 1})
...
def direct_parents(iri: str) -> list[TermCandidate]:
    ...
    with _client() as c:
        r = c.get(f"{OLS_BASE}/ontologies/efo/terms/{frag}/hierarchicalParents", params={"size": 50})
        if r.status_code == 404:
            return []
        r.raise_for_status()
        terms = r.json().get("_embedded", {}).get("terms", [])
